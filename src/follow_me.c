...
static u8 DetermineFollowerState(struct ObjectEvent* follower, u8 state, u8 direction)
{
    ...
    case MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT ... MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_DOWN_RIGHT:
        ...
        {
            newState = MOVEMENT_INVALID;
            RETURN_STATE(state, DIR_SOUTH); //Each is its own movement
        }
        else if (newState - direction >= MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT
         && newState - direction <= MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_DOWN_RIGHT) //Change direction on stairs
        {
            u8 simpleState;
            ...
            newState -= direction;
            simpleState = MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT;
            switch (newState)
            {
            case MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT ... MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_DOWN_RIGHT:
                state = (newState - MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT) + simpleState;
                break;
            case MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT_RUNNING ... MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_DOWN_RIGHT_RUNNING:
                state = (newState - MOVEMENT_ACTION_WALK_STAIRS_DIAGONAL_UP_LEFT_RUNNING) + simpleState;
                break;
            case MOVEMENT_ACTION_RIDE_WATER_CURRENT_UP_LEFT ... MOVEMENT_ACTION_RIDE_WATER_CURRENT_DOWN_RIGHT:
                state = (newState - MOVEMENT_ACTION_RIDE_WATER_CURRENT_UP_LEFT) + simpleState;
                break;
            }
